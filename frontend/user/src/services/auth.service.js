...
class AuthService {
    ...
    async register(user) {
        try {
            const data = (await this.api.post("/register/reader", user)).data
            return {
                status: "success",
                message: data.message || "Reader registered successfully",
                data: data.data,
            }
    ...
    async logout() {
        try {
            ...
            return {
                status: "success",
                message: data.message || "Reader logged out successfully",
            }
        } catch (err) {
            return {
                status: "error",
